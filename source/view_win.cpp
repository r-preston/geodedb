...
const char * szTitle = "Application";
...
public:
  ViewWin(int width, int height) {
    ...
    hwnd_ = CreateWindow(reinterpret_cast<LPCSTR>(szWindowClass),
              szTitle,
              WS_OVERLAPPEDWINDOW,
              CW_USEDEFAULT,
              CW_USEDEFAULT,
  ...
  static void PlatformInit() {
    ...
    wc.lpszClassName = reinterpret_cast<LPCSTR>(szWindowClass);
  ...
  static ViewWin* GetFromHandle(HWND handle) {
      for (std::vector<ViewWin*>::iterator i = g_active_views_.begin(); i != g_active_views_.end(); i++)
      {
        if ((*i)->hwnd() == handle)
        {
             return *i;
  ...
  virtual void OnChangeTitle(Awesomium::WebView* caller,
    ...
    //LPCWSTR lp_title_wide = title_wide.c_str();

    SetWindowText(hwnd_, title_utf8.c_str());
  }
